    ...
    (props
     : Props) =
        >
        {
            ...
            const teams =
                [
                  {id : "team1", display_name : "team name"},
                  {id : "team2", display_name : "team name 2"},
                ]
            ...
            const dispatch = useDispatch()
                ...
                dispatch(loadTeams())
                    ...
                    const teamsData =
                        useSelector<GlobalState, IDMappedObjects<Team>>(
                            (state) = > getTeams(state),
                            (prev,
                             curr) = > shallowEqual(Object.keys(prev).sort(),
                                                    Object.keys(curr).sort()))
            // console.log(teamsData)

            const team = (props
                          : teams_prop) =
                > (<div className = 'team' key = {props.id} style =
                        {{display : "flex", padding : 10}}>
                   <div style = {{flexGrow : 50, margin : 1}}>
                   <select name = "pets" id =
                        "pet-select" className = {"form-control"}>

                   <option value = ""> --Please choose an team--</ option>
                   <option value = "dog">
                       Dog</ option><option value = "cat">
                           Cat</ option><option value = "hamster">
                               Hamster</ option><option value = "parrot">
                                   Parrot</ option><option value = "spider">
                                       Spider</ option>
                   <option value = "goldfish">
                       Goldfish</ option></ select></ div>
                   <div style = {{flexGrow : 0.5, margin : 1}}>
                   <input type = {"number"} name = {props.id} className =
                        {"form-control"} /></ div>
                   ...
                   <Button>{'Remove'}</ Button></ div>
                                   ...
                                   <div><h3>{"Team Setting"}</ h3>
                                   <div className = 'mt-2'>{
                                       "Set specific team rules."}</ div></ div>
                                   <div className = 'button'>
                                   <a className = {'btn btn-primary'}
                                    // onClick={handleOpen}
                                    >{"Add Teams"}</ a></ div></ div>{teams.map(
                                       (team_data) = > team({
                                                       id : team_data.id,
                                                       display_name : team_data
                                                           .display_name
                                                     }, ))}</ div></ div>)
